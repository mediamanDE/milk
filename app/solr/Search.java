...
public class Search {
	...
	public static List<Message> SearchMessageAll(String search, boolean lazy) {
		...
		if (queryRsp != null) {
			...
			// GET Highlight
			/*
			 * Iterator<SolrDocument> iterResult =
			 * queryRsp.getResults().iterator(); while (iterResult.hasNext()) {
			 ...
			 * uniqueKey field
			 * 
			 * if (queryRsp.getHighlighting().get(id) != null) { List<String>
			 * highlightSnippets =
			 * queryRsp.getHighlighting().get(id).get("content"); } }
			 */
	...
	private static Message getCleanMessage(SolrDocument solrDocument) {
		...
		message.setMessagetext((String) solrDocument
				.getFieldValue("description"));
		// FIX
		// message.setPostdate(new
		// Date((String)solrDocument.getFieldValue("last_modified")));
		return message;
	}
	...
	private static QueryResponse queryFromServer(SolrQuery query) {
		...
		if ( !enabled ) {
			Logger.info("Tried to search Solr, while it's disabled");
			return null;
		}		
		...
		try {
			...
			Logger.warn("Exception while searching Solr", e);
		}
		return queryRsp;
	}
}
